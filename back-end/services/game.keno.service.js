...
class GameKenoService {
  ...
  randomKetQua = async () => {
    ...
    // Tự động can thiệp kết quả, bên nhiều hơn sẽ thua
      ...
      for (let i = 1; i <= 5; i++) {
        if (
          tongTienCuocGame[i]["C"] === tongTienCuocGame[i]["L"] &&
          tongTienCuocGame[i]["L"] === tongTienCuocGame[i]["T"] &&
          tongTienCuocGame[i]["T"] === tongTienCuocGame[i]["X"]
        ) {
          ...
          if (randomNumber === 0) {
            ...
            tempResult[i - 1] = randomBiTheoLoai({ loai: "T" });
          } else if (randomNumber === 3) {
            tempResult[i - 1] = randomBiTheoLoai({ loai: "X" });
          }
          continue;
        }
        // Tìm min
